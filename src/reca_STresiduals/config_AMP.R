...
config$substrate_raster <- env_or_default(
  ...
  file.path(config$data_dir, "SUBS_brk.grd")
)
...
config$species_group <- env_or_default("RECA_SPECIES_GROUP","AMP")

## Input observations CSV filename (relative to data_dir)
config$obs_csv <- env_or_default("RECA_OBS_CSV", "ala_AMP_2026-03-03.csv")
