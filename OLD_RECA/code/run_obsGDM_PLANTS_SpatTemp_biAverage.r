...
lst <- c("ffbase","raster","SDMTools","nnls","lubridate","doSNOW","foreach","parallel")

lapply(lst,function(x){install.packages(x)})
...
#
s1 <- Sys.time()
load("PLANTS_aggregated_basicFilt.RData")
...
date_test <- as.Date(datRED$eventDate) > (as.Date("1911-01-01") %m+% years(60))
...
c_yrs <- seq(62,75,by=2) ## climate time frame
...
for(c_yr in c_yrs){
	for(w_yr in w_yrs){
		...
		ext_data <- obsPairs_out[,2:9]
		nMatch <- 1000000
		save_prefix <- paste("biAverage_","PLANTS_","1mil_",c_yr,"climYr_",w_yr,"weathYr_",sep="")
...
if(TRUE){
	...
	init_params <- list(
		...
		list(variables = c("FD_191101-201712", "WRel1_191101-201712"),
			mstat = 'mean', 
			cstat = 'min', 
			window = c_yr,
			prefix=paste("MinXbr",c_yr,sep="_")),				
		list(variables = c("mean_PT_191101-201712", "FWPT_191101-201712"),
			mstat = 'mean', 
			cstat = 'max', 
			window = c_yr,
		...
		list(variables = c("TNn_191101-201712", "TXx_191101-201712"),
			...
			window = c_yr,
			prefix=paste("MaxXbr",c_yr,sep="_")),		
		list(variables = c("FD_191101-201712", "PD_191101-201712"),
			mstat = 'mean', 
			cstat = 'max', 
			window = c_yr,
			prefix=paste("MaxXbr",c_yr,sep="_")),
...
##here	
	...
	subs_brk <- brick("SUBS_brk_VAS.grd")
	...
	names(Trng_1) <- c(paste("Trng_",c_yr,"_191101-201712_1",sep=""),paste("Trng_",w_yr,"_191101-201712_1",sep=""))
	...
	names(Trng_2) <- c(paste("Trng_",c_yr,"_191101-201712_2",sep=""),paste("Trng_",w_yr,"_191101-201712_2",sep=""))
...
test <- is.na(rowSums(obsPairs_out[,23:98]))
...
toSpline <- toSpline[,-c(1,8,11:20,29,36,39,46,49:58,67,74)]
...
f1 <- paste(colnames(mod_ready)[-1],collapse="+") ## fformula[1:i]
