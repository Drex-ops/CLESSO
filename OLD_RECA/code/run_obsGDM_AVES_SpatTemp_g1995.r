...
date_test <- as.Date(datRED$eventDate) >= (as.Date("1995-01-01")) #%m+% years(45))
...
registerDoSEQ()
biol_group="AVES"
fn=paste("ObsPairsTable_RECA_",biol_group,"WindowTestRuns_g1995.RData",sep="")
...
c_yrs <- seq(63,84,by=2) ## climate time frame
...
for(c_yr in c_yrs){
	for(w_yr in w_yrs){
		...
		nMatch <- 1000000
		save_prefix <- paste("AVESg1995_","1mil_",c_yr,"climYr_",w_yr,"weathYr_",sep="")
