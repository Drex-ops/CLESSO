...
\title{To DO: ObsPair modelling v2}

\begin{list}{label}{spacing}
    - remove interceot from Beta
...
\begin{itemize}
    ...
    \iten Create an observation-pair dataset $P$ by sampling pairs of occurrence records from a set of species observations for your region of interest.
\end{itemize}
...
\begin{equation}
    S_{i,j} = \frac{2\gamma_{i,j}}{\alpha_{i} + \alpha{j}}
\end{equation}

and rearrange to $\gamma_{i,j} = S_{i,j}(\alpha_{i} + \alpha{j})/2$ then substitute it into (3) we get:
...
The turnover model then follows the GDM style as:

\begin{equation}
    \eta_{i,j} = f_{d}(d_{i,j}) + \sum_{l=1}^{m}f_l(\Delta x_{l,i,j})
\end{equation}
...
where $f_{d}$ and $f_{l}$ are monotone splines to ensure turnover increases with increasing environmental and geographic distance 
