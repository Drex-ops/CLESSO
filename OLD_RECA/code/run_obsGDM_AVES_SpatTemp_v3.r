...
c_yrs <- c(51)#seq(46,50,by=2) ## climate time frame
...
save_prefix <- paste("V3_",save_prefix,sep="")
...
splined_new <- cbind(splined_1,splined_2,splined_subs)

splined_new <- splined_new[same_site | same_time,]

#splined <- splined[,1:3] ## testing

## fit model
mod_ready <- cbind(Match=obsPairs_out$Match[same_site | same_time],as.data.frame(splined_new)) ## make table
