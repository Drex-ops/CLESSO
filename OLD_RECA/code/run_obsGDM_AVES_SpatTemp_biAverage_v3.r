...
c_yrs <- c(seq(61,75,by=2))#seq(46,50,by=2) ## climate time frame
...
for(c_yr in c_yrs){
	for(w_yr in w_yrs){
		...
		files <- list.files()
		get_env <- !any(grepl(paste(save_prefix,"ObsEnvTable.RData",sep=""),files))
...
splined_new <- splined[same_site | same_time,]
