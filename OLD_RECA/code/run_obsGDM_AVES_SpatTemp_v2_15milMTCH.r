...
registerDoSEQ()
	...
	fn=paste("ObsPairsTable_RECA_",biol_group,"16Mil_MTCHS.RData",sep="")
...
c_yrs <- c(20,25,30,35,40,45,50,55,60,65) ##seq(10,75,by=2) ## climate time frame
...
for(c_yr in c_yrs){
	for(w_yr in w_yrs){
		...
		ext_data <- obsPairs_out[,2:9]
		nMatch <- 1000000
		save_prefix <- paste("AVES_","1_5mil_",c_yr,"climYr_",w_yr,"weathYr_",sep="")
...
if(!get_env){load(paste(save_prefix,"ObsEnvTable.RData",sep=""))}

save_prefix <- paste("V2",save_prefix,sep="_")
...
spat_temp <- rep(0,nrow(obsPairs_out))
spat_temp[same_site] <- 1
spat_temp[same_time] <- 2
...
toSpline_subs <- toSpline[,c(10:17,38:45)]
toSpline_env <- toSpline[,-c(10:17,38:45)]
...
splined_env <- splineData(toSpline_env)
splined_subs <- splineData(toSpline_subs)

splined_0 <- splined_env ## spat temp - all vars
splined_1 <- splined_env ## temp - only climate
splined_2 <- splined_env ## spat - all vars

splined_0[same_site | same_time,] <- 0
colnames(splined_0) <- paste("ST",colnames(splined_0),sep="_")
splined_1[!same_site,] <- 0
colnames(splined_1) <- paste("T",colnames(splined_1),sep="_")
splined_2[!same_time,] <- 0
colnames(splined_2) <- paste("S",colnames(splined_2),sep="_")

splined_new <- cbind(splined_0,splined_1,splined_2,splined_subs)


#splined <- splined[,1:3] ## testing

## fit model
mod_ready <- cbind(Match=obsPairs_out$Match,as.data.frame(splined_new)) ## make table
...
fit$observed <- obsPairs_out$Match
...
print(paste('Nagelkerke: ',gdm_dev$Nagelkerke))
