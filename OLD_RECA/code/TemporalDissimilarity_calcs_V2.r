...
smp_idx <- 1:nrow(crds)
idx <- sample(smp_idx,nSamples)

ext_points <- crds ##[idx,]


## get model and quantiles
load("//lw-osm-02-cdc/OSM_CBR_LW_BACKCAST_work/DEV/hos06j/AVES_1mil_50climYr_3weathYr_ObsEnvTable.RData")
load("//lw-osm-02-cdc/OSM_CBR_LW_BACKCAST_work/DEV/hos06j/AVES_1mil_50climYr_3weathYr__coefficients.RData")
...
for(i in 1:length(yrs)){
	...
	toSpline <- cbind(
						env_out1[,1:9],env1_subs,Trng_1,env_anom_out1,
						env_out2[,1:9],env2_subs,Trng_2,env_anom_out2)
...
save(dissimilarities_cum,file=paste(nSamples,"dissimilarities_cum.RData",sep=""))
save(dissimilarities,file=paste(nSamples,"dissimilarities_yrONyr.RData",sep=""))
...
png("Dissimilarities_yrOnyr.png")
...
save(dissimilarities_epoch,file=paste(nSamples,"dissimilarities_epoch.RData",sep=""))
...
png("Dissimilarities_1965Epoch.png")
