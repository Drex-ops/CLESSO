...
setwd("//lw-osm-02-cdc/OSM_CBR_LW_BACKCAST_work/DEV/hos06j")
write.csv(datRED,"VAS_aggregated_basicFilt.csv",row.names=FALSE)
save(datRED,file="VAS_aggregated_basicFilt.RData")

### load datRED from siteAggregator process
#
s1 <- Sys.time()
load("VAS_aggregated_basicFilt.RData")
...
save_prefix <- paste("VAS_",nMatch/1000000,"mil_",idx1,"_run",run,"_",sep="")
...
run <- 2
nMatches <- c(rep(50000,10),rep(100000,10),rep(150000,10),rep(250000,10))
...
run <- 3
nMatches <- c(rep(500000,10),rep(1000000,10),rep(1500000,10),rep(2000000,10))
...
run <- 4
nMatches <- c(rep(2500000,10),rep(3000000,10),rep(1750000,10),rep(2750000,10))
